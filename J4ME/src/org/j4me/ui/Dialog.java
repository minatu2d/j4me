...
public abstract class Dialog
    ...
    public int max_number_component = 2;
    /**
    ...
    public int real_number_component = 0;
    private boolean layoutJustCalculated = false;

    /**
     * Implicitly called by derived classes to setup a new J4ME form.
     */
    public Dialog(int max_len) {
        super();
        this.setMaxNumbComponet(max_len);
    ...
    private void setMaxNumbComponet(int max_len) {
        this.max_number_component = max_len;
    }
    ...
    public void append(Component component) {
        ...
        if (absoluteHeights == null) {
            ...
            calculateAppendLayout(UIManager.getTheme(), getWidth(), getHeight());
        }
    }
    ...
    public void setSelected(int index) {
        ...
        int maxScroll = absoluteHeights[real_number_component] - getHeight();
    ...
    public /*synchronized*/ void paint(Graphics g) {
        ...
        for (int i = 0; i < real_number_component; i++) {
            ...
            if (componentTop >= bottomOfScreen - 13) {
                // Skip drawing components below the screen.
                c.visible(false);
            } else if (componentBottom <= topOfScreen + 13) {
    ...
    public void setSpacing(int spacing) {
        if ((spacing >= 0) && (this.spacing != spacing)) {
            ...
            //System.out.println("Set spacing");
...
//        }
        ...
        if (absoluteHeights[real_number_component] > screenHeight) {
            if (layoutJustCalculated) {
                // System.out.println("Calculate for scroll");
                // Recalculate the layout now that we're adding a vertical scrollbar.
                formWidth -= theme.getVerticalScrollbarWidth();
                calculateLayoutForScrollbar(theme, formWidth, screenHeight);
            }
            return true;
        } else {
            return false;
    ...
    public void invalidate() {
        // System.out.println("Change invalidated");
     ...
     */
    public void calculateLayout(Theme theme, int width, int height) {
        //System.out.println("First time");
        if (componentWidths == null) {
            componentWidths = new int[max_number_component + 1];  // Dummy element at bottom for end of components
            absoluteHeights = new int[max_number_component + 1];
        }
        ...
        real_number_component = size;
        // Add a dummy last element to record the bottom of all components.
        absoluteHeights[real_number_component] = componentY;
    }
     ...
     */
    public int nextHighlightableComponent(boolean down, int maxScroll) {
        if (down) {
    ...
    public void setMenuText(String left, String right) {
        ...
        //System.out.println("Set menu text");
        invalidate();
    }
    ...
    public void pointerPressed(int x, int y) {
        ...
        if (hasVerticalScrollbar()) {
            ...
            if (x >= scrollbarX) {
                ...
                int height = getHeight();
                int formHeight = absoluteHeights[real_number_component];
    ...
    /*Calculate layout when a element is added*/
    public void calculateAppendLayout(Theme theme, int width, int height) {
        //int componentY = margin;  // First component is "margin" from the top

        // Scroll through the components determining the vertical position of each one.
        int size = components.size();
        Component c = (Component) components.lastElement();
        int[] dimensions = c.getPreferredSize(theme, width, height);
        absoluteHeights[size] = absoluteHeights[size - 1] + dimensions[1] + spacing;
        componentWidths[size - 1] = dimensions[0];
        real_number_component = size;
        layoutJustCalculated = true;
//        Enumeration list = components.elements();
//
...
//        }
//
//        real_number_component = components.size();
//        // Add a dummy last element to record the bottom of all components.
//        absoluteHeights[real_number_component] = componentY;    
    }
     ...
     * @param height is the number of pixels high the screen is.
     */
    public void calculateLayoutForScrollbar(Theme theme, int width, int height) {
//        componentWidths = new int[20];  // Dummy element at bottom for end of components
//        absoluteHeights = new int[20];
        int componentY = margin;  // First component is "margin" from the top
